...
// sgMail.setApiKey(
//   "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"
// );
...
//     const jobsData = await JobModel.findOne(
//       {
//         year: `${year}-${nextYear}`,
//       },
//       "data"
...
//       const matchingJobData = jobsData.data.find(
//         (job) => job.importerURL === reportField.importerURL
//       );

//       if (matchingJobData) {
//         const pendingJobs = matchingJobData.jobs.filter((job) => job);

//         const worksheetName = reportField.importer; // Truncate worksheet name if needed
...
//         const countArrivalDateSize20 = matchingJobData.jobs.filter(
//           (job) =>
...
//         const countArrivalDateSize40 = matchingJobData.jobs.filter(
//           (job) =>
//             job.status.toLowerCase() === "pending" &&
//             job.container_nos.some(
...
//         const countNoArrivalDateSize20 = matchingJobData.jobs.filter(
//           (job) =>
//             job.status.toLowerCase() === "pending" &&
...
//         const countNoArrivalDateSize40 = matchingJobData.jobs.filter(
//           (job) =>
//             job.status.toLowerCase() === "pending" &&
